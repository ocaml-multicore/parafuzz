...
let current_state = ref { chan = src_of_seed (Random.int64 (Int64.max_int));
...
let shuffle l =
  ...
  js |> List.iteri (fun i j ->
      let t = arr.(i) in
      arr.(i) <- arr.(j); arr.(j) <- t);
  Array.fold_right (fun elem l -> elem::l) arr []
...
let sample_from_generator gen =
  let v, _ = generate 100 !current_state gen in
  v
